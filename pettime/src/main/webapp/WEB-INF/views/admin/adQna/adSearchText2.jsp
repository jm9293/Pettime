		...
		<div class="box_ul">
			<ul class="box_li">
				<c:choose>
					<c:when test="${page -1 < 1 }">
						<li class="paging"><a
					href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=1&title=${title}">이전</a></li>
					</c:when>
					<c:otherwise>
						<li class="paging"><a
						href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=${page -1}&title=${title}">이전</a></li>
					</c:otherwise>
				</c:choose>
				<c:if test="${pageNum > 0 }">
					...
					href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=${item2 }&title=${title}">${item2 }</a></li>
						</c:when>
						<c:otherwise>
							<li class="paging"><a
					href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=${item2 }&title=${title}">${item2 }</a></li>
			...
			<c:choose>
				<c:when test="${page < pageNum }">
					<li class="paging"><a
					href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=${page + 1 }&title=${title}">다음</a></li>
				</c:when>
				<c:otherwise>
					<li class="paging"><a
					href="${pageContext.request.contextPath }/admin/adQna/adSearchText2?page=${pageNum}&title=${title}">다음</a></li>
				</c:otherwise>
			</c:choose>
			</ul>
		</div>
